    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white flex items-center">
          <BookText size={28} className="mr-3 text-indigo-600 dark:text-indigo-400" />
          My Interview Notes
        </h1>
      ...
      {sessionsWithNotes.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
          <BookText size={48} className="mx-auto text-slate-400 dark:text-slate-600" />
          <h2 className="mt-4 text-xl font-semibold text-slate-700 dark:text-slate-200">No Notes Found</h2>
          ...
          {sessionsWithNotes.map((session, index) => (
            <div key={index} className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
              <div className="flex items-center gap-3 pb-4 border-b border-slate-200 dark:border-slate-700 mb-4">
                ...
                <div>
                  <h2 className="font-semibold text-lg text-slate-800 dark:text-white">{session.type}</h2>
              ...
              <div className="space-y-4">
                {session.transcript.map((entry, entryIndex) => (
                  <div key={entryIndex} className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                    <p className="font-semibold text-slate-700 dark:text-slate-300 flex items-start gap-2">
                      <MessageSquare size={16} className="mt-1 flex-shrink-0 text-indigo-500 dark:text-indigo-400" />
                      <span>{entry.question}</span>
                    </p>
                    <div className="mt-2 pl-6 border-l-2 border-indigo-200 dark:border-indigo-800 ml-2">
                      <p className="text-slate-600 dark:text-slate-400 whitespace-pre-wrap">{entry.notes}</p>
                    </div>
                  </div>
                ))}
              </div>
